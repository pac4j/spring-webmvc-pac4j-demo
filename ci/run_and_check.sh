...
# Script to launch spring-webmvc-pac4j-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting spring-webmvc-pac4j-demo..."
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 spring-webmvc-pac4j-demo test completed successfully!"
    ...
    echo "💥 spring-webmvc-pac4j-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
